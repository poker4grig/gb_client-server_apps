...
class ClientDatabase:
    ...
    # Конструктор базы данных клинта
    def __init__(self, name):
        ...
        # мультипоточный необходимо отключить проверки на подключения с разных
        # потоков, иначе sqlite3.ProgrammingError
        path = os.path.dirname(os.path.realpath(__file__))
        filename = f'client_{name}.db3'
        self.database_engine = create_engine(f'sqlite:///'
        ...
        # Связываем Python сущности таблиц и таблицы БД
        ...
        # Очистка таблицы контактов, т.к. при запуске они подгружаются с
        # сервера
    ...
    def del_contact(self, contact):
        ...
        # Добавление известных пользователей. Первоначально таблица очищается,
        # т.к. пользователи берутся только с сервера
