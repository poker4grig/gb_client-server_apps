...
def gui_create_model(database):
    ...
    # содержимое из таблицы юзеров
    list_.setHorizontalHeaderLabels(['Имя Клиента', 'IP Адрес', 'Порт',
                                     'Время подключения'])
    for row in list_users:
        user, ip, port, time = row
        user = QStandardItem(user)
        user.setEditable(False)  # Запрещаем редактирование
        ip = QStandardItem(ip)
...
class HistoryWindow(QDialog):  #  Класс окна с историей пользователей
    def __init__(self):
        super().__init__()
        self.initUI()
    ...
    def initUI(self):
        ...
        # список, содержащий саму историю
