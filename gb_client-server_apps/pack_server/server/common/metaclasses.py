"""Модуль, содержащий метаклассы, для контроля классов приложения."""

import dis


class ServerMaker(type):
    """Метакласс проверки данных классов.

    Проверяет, что в результирующем классе нет клиентских вызовов таких как:
    connect. Также проверяется, что серверный сокет является TCP и работает
    по IPv4 протоколу.
    """
...
class ClientMaker(type):
    ...
    Проверяет, что в результирующем классе нет серверных вызовов таких как:
    accept, listen. Также проверяется, что сокет не создаётся внутри
    конструктора класса.
    """
